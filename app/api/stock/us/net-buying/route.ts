  ...
  try {
    ...
    if (data.length === 0) {
      ...
      if (!KIS_APPKEY || !KIS_APPSECRET) {
        ...
        headers.set("x-debug-reason", "Real-time KIS US net buying returned 0 items. Possibly outside US market hours (or Yahoo fallback failed) and DB cache is empty.");
      }
    } else {
      headers.set("x-debug-status", "success");
      headers.set("x-debug-reason", "US net buying data loaded successfully.");
    }
    return NextResponse.json(data, { headers });
  } catch (err: any) {
    headers.set("x-debug-status", "error");
    headers.set("x-debug-reason", `Server error: ${err.message || err}`);
    return NextResponse.json({ error: "Failed to fetch US net buying", message: err.message || String(err) }, { status: 500, headers });
  }
}
