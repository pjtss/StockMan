...
export async function DELETE(_: Request,{params}:{params:Promise<{id:string}>}) { if (!(await requireAdminSession())) return NextResponse.json({error:"UNAUTHORIZED"},{status:401}); await deleteInquiry(Number((await params).id)); return NextResponse.json({ok:true,deleted:true}); }
