...
export async function POST(request: Request) {
  ...
  const settings = await loadFeatureModuleSettings("us-product-classification");
  ...
  const latest = await loadLatestExecutedAutomationRun("us-product-classification").catch(() => null);
  ...
  if (elapsedSeconds != null && elapsedSeconds < intervalSeconds) {
    await recordSkippedAutomationRun("us-product-classification", "outside_interval", { intervalSeconds, elapsedSeconds: Math.round(elapsedSeconds) });
    return NextResponse.json({ ok: true, skipped: true, reason: "outside_interval", intervalSeconds, elapsedSeconds: Math.round(elapsedSeconds) });
  }
  try { return NextResponse.json(await withAutomationRun("us-product-classification", () => refreshUsProductClassifications({ concurrency: 4 }))); }
