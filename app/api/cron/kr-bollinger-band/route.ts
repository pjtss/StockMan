...
export async function POST(request: Request) {
  ...
  if (!settings.enabled) { await recordSkippedAutomationRun("kr-bollinger-band", "disabled"); return NextResponse.json({ ok: true, skipped: true, reason: "disabled" }); }
  if (!isWithinSchedule(settings, new Date())) { await recordSkippedAutomationRun("kr-bollinger-band", "outside_schedule"); return NextResponse.json({ ok: true, skipped: true, reason: "outside_schedule" }); }
  ...
  const result = await withAutomationRun("kr-bollinger-band", async () => {
    const scan = await scanStoredKrBollingerBands();
    const discord = await sendKrBollingerBandSignals(scan.results);
    return { ...scan, discord };
  });
  return NextResponse.json({ ok: true, intervalSeconds, data: result });
}
