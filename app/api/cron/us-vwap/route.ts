...
async function handle(request: Request) {
  ...
  const flags = await loadAdminFeatureFlags(); const settings = await loadFeatureModuleSettings("us-turnover-ratio");
  ...
  return NextResponse.json({ ok: true, data: await withAutomationLock("us-vwap", runUsVwapAutomation) });
}
