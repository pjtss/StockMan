...
export async function POST(request: Request) {
  ...
  if (!settings.enabled) { await recordSkippedAutomationRun("us-bollinger-middle-lower", "disabled"); return NextResponse.json({ ok: true, skipped: true, reason: "disabled" }); }
  ...
  return NextResponse.json({ ok: true, intervalSeconds, cooldownSeconds: settings.cooldownSeconds, effectiveIntervalSeconds, ...result });
}
