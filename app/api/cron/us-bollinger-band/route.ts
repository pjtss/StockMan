...
export async function POST(request: Request) {
  ...
  if (elapsedSeconds != null && elapsedSeconds < intervalSeconds) {
    ...
    await recordSkippedAutomationRun("us-bollinger-band", reason, { intervalSeconds, elapsedSeconds: Math.round(elapsedSeconds), latestRunStatus: latestRun?.status });
    return NextResponse.json({ ok: true, skipped: true, reason, intervalSeconds, elapsedSeconds: Math.round(elapsedSeconds) });
  }
  try {
