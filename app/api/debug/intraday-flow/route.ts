...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, generatedAt: new Date().toISOString(), worker: getIntradayWorkerSnapshot(), flow: { snapshot: { currentCount: state.current.length, previousCount: state.previous.length }, filters: { managedCandidates: state.candidates.length, ratioEligible: state.candidates.filter((candidate) => candidate.priority > 0).length, mvpTracking: mvpCandidates.length, mvpQualified: mvpQualified.length, mvpThreshold: INTRADAY_MVP_POLICY.threshold, mvpWindowMinutes: INTRADAY_MVP_POLICY.windowMs / 60_000, mvpRequiredSamples: INTRADAY_MVP_POLICY.requiredSamples }, queue: { dueCount: intradayMemoryState.dueCandidates().length, inflightCount: state.inflightCount } }, metrics, state });
}
