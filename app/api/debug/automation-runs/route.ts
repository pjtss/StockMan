...
export async function GET(request: Request) {
  ...
  if (!cronAuthorized && !(await requireAdminSession())) return json({ error: "Unauthorized" }, { status: 401 });
  ...
  try {
    return json(await loadAutomationDebugSnapshot({
    ...
    return json({ ok: false, stage: "load_automation_runs", ...diagnostics, checkedAt: new Date().toISOString() }, { status: isSchemaError(error) ? 503 : 500 });
  }
}
