...
export async function GET(request: Request) {
  ...
  const policy: Partial<UsBollingerPolicy> = { timeframe: params.get("timeframe") === "W" || params.get("timeframe") === "M" ? params.get("timeframe") as "W" | "M" : "D", period: number("period"), stdDevMultiplier: number("stdDevMultiplier"), minPrice: number("minPrice"), minVolume: number("minVolume"), minTurnoverRatio: number("minTurnoverRatio"), zone: "MIDDLE_TO_LOWER" };
  ...
  try { return NextResponse.json({ mode: "ADMIN_MANUAL_TEST", ...(await scanStoredUsBollingerBands({ moduleKey: "us-bollinger-middle-lower", policy })) }); }
