...
export async function POST(request: Request) {
  ...
  if (interaction.data.name === "turnover-list") {
    ...
    void scanUsDailyTrend().then(async (result) => { const webhookStatus = await notifyDailyWebhook("일봉 급등 추세 통합", () => sendUsDailyTrendToDiscord(result.results)); return updateOriginalResponse(applicationId, interaction.token, `${formatDailyTrendResult(result)}\n\n${webhookStatus.trim()}`); }).catch(() => updateOriginalResponse(applicationId, interaction.token, "일봉 급등 추세 종목을 조회하는 중 오류가 발생했습니다."));
      ...
      void analyzeUsShortSqueeze(ticker).then((result) => updateOriginalResponse(applicationId, interaction.token, result.ok ? [`🩳 **${result.ticker} 숏스퀴즈 평가**`, `상태 ${result.squeezeState} · 등급 ${result.squeezeGrade} · 점수 ${result.squeezeScore}/${result.maxAvailableScore}`, `신뢰도 ${result.dataConfidence} · 커버리지 ${result.scoreCoveragePercent?.toFixed(1) ?? "-"}%`, `현재가 ${result.currentPrice ?? "-"} · SI/Float ${result.shortInterestFloatPercent == null ? "-" : `${result.shortInterestFloatPercent.toFixed(2)}%`}`, `Days To Cover ${"daysToCover" in result && typeof result.daysToCover === "number" ? result.daysToCover.toFixed(2) : "-"} · Short Volume Ratio ${result.shortVolumeRatio == null ? "-" : `${(result.shortVolumeRatio * 100).toFixed(1)}%`}`, `공매도 잔고 출처 ${result.shortInterestSource ?? "-"} · 유통주 출처 ${result.floatSource ?? "-"} (${result.floatDataType ?? "-"})`, `기준일 공매도 ${result.shortInterestAsOf ?? "-"} · 유통주 ${result.floatAsOf ?? "-"}`, "※ 공개 데이터 기반 추정이며 실제 강제청산 가격·숏 원가는 제공되지 않습니다."].join("\n") : `공매도 평가 실패: ${result.error}`)).catch((error) => updateOriginalResponse(applicationId, interaction.token, `공매도 평가 실패: ${error instanceof Error ? error.message : "알 수 없는 오류"}`));
