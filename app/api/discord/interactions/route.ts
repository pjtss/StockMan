...
export async function POST(request: Request) {
  ...
  if (interaction.type !== 2 || !["ticker", "daily-breakout", "mfi-oversold", "dmi", "macd"].includes(interaction.data?.name)) return NextResponse.json({ type: 4, data: { content: "지원하지 않는 명령어입니다.", flags: 64 } });
