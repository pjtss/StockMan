...
export function generateStaticParams() { return FEATURE_MODULES.map((module) => ({ key: module.key })); }
...
export default async function FeatureModulePage({ params }: { params: Promise<{ key: string }> }) {
  ...
  return <AdminPageShell eyebrow="FEATURE MODULE" title={module.label} description={module.description}>
    ...
    <section className={styles.links}><h2>기능 전용 설정</h2><p>공통 운영 설정과 분리된 기능별 조건은 각 전용 화면에서 관리합니다.</p>{module.key === "sec-realtime" && <Link href="/admin/sec-test">SEC 분석·평가 설정 →</Link>}{module.key === "us-scanners" && <Link href="/admin/api-config">KIS 요청 파라미터 →</Link>}</section>
  </AdminPageShell>;
}
