...
export async function recommendMultiTimeframe(market: "KR" | "US", mode: Mode = "all", limit = 30) {
  ...
  const candles = await db.execute(sql.raw(`SELECT c.market, c.code, c.timeframe, c.candle_date AS date, c.open, c.high, c.low, c.close, c.volume, c.fetched_at AS "updatedAt" FROM ${candlesTable} c JOIN ${universeTable} u ON u.market = c.market AND u.code = c.code WHERE c.timeframe IN ('D','W','M') AND c.close IS NOT NULL AND u.enabled = true AND u.daily_active = true AND u.instrument_type = 'COMMON_STOCK' ${market === "KR" ? "AND COALESCE(u.is_suspended, false) = false AND COALESCE(u.trading_halt_code, '') NOT IN ('Y','1') AND COALESCE(u.liquidation_code, '') NOT IN ('Y','1') AND COALESCE(u.managed_issue_code, '') <> 'Y'" : "AND COALESCE(u.is_etf, false) = false AND COALESCE(u.is_warrant, false) = false AND COALESCE(u.is_derivative, false) = false AND COALESCE(u.is_dr, false) = false AND COALESCE(u.is_leveraged, false) = false AND COALESCE(u.is_inverse, false) = false"} ORDER BY c.market, c.code, c.timeframe, c.candle_date`));
