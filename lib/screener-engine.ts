...
export function evaluateScreenerFilters(metrics: Record<string, any>, request: ScreenerRequest) {
  ...
  const matched = conditions.length === 0
    ? true
    : request.logic === "OR"
      ? conditions.some((x) => x.passed)
      : conditions.every((x) => x.passed);
