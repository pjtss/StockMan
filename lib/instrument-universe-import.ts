...
import { instrumentUniverseSyncRuns, krCommonStockUniverse, krSpecialInstrumentUniverse, usCommonStockUniverse, usSpecialInstrumentUniverse } from "@/lib/schema";
...
type UsRow = { market: "NAS" | "NYS" | "AMS"; code: string; realtimeSymbol: string; name: string; englishName: string; securityType: string; etpType: string; currency: string; countryCode: string; industryCode: string; isEtf: boolean; isLeveraged: boolean; isInverse: boolean; isWarrant: boolean; isDerivative: boolean; isDr: boolean; sourceFile: string; rawPayload: string };
...
export function parseOverseas(buffer: Buffer, market: "NAS" | "NYS" | "AMS", sourceFile: string): UsRow[] {
  return decode(buffer).split(/\r?\n/).map((raw) => raw.replace(/\r$/, "")).filter(Boolean).map((raw) => {
    ...
    const isEtf = securityType === "3" || ["001", "005"].includes(etpType);
    const isWarrant = securityType === "4";
    const isDerivative = securityType === "4" || ["002", "003", "006"].includes(etpType);
    return { market, code: f[4] ?? "", realtimeSymbol: f[5] ?? "", name: f[6] ?? "", englishName: f[7] ?? "", securityType, etpType, currency: f[9] ?? "", countryCode: f[0] ?? "", industryCode: f[23] ?? "", isEtf, isLeveraged: false, isInverse: false, isWarrant, isDerivative, isDr: bool(f[20] ?? ""), sourceFile, rawPayload: raw };
...
export async function importInstrumentMasters(sourceDirectory: string) {
  ...
  try {
    ...
    for (let offset = 0; offset < uniqueKrRows.length; offset += 250) { const rows = uniqueKrRows.slice(offset, offset + 250).map((row) => ({ ...row, ...classifyKr(row), enabled: true, lastSeenAt: now, updatedAt: now })); const common = rows.filter((row) => row.instrumentType === "COMMON_STOCK" && !row.isSuspended && !["Y", "1"].includes(row.managedIssueCode)); const special = rows.filter((row) => !common.includes(row)); const upsert = async (table: typeof krCommonStockUniverse, batch: typeof common) => { if (!batch.length) return; await db.insert(table).values(batch).onConflictDoUpdate({ target: [table.market, table.code], set: { standardCode: sql`excluded.standard_code`, name: sql`excluded.name`, instrumentType: sql`excluded.instrument_type`, securityGroupCode: sql`excluded.security_group_code`, marketCapScale: sql`excluded.market_cap_scale`, industryLargeCode: sql`excluded.industry_large_code`, industryMediumCode: sql`excluded.industry_medium_code`, industrySmallCode: sql`excluded.industry_small_code`, etpProductClassCode: sql`excluded.etp_product_class_code`, preferredClassCode: sql`excluded.preferred_class_code`, tradingHaltCode: sql`excluded.trading_halt_code`, liquidationCode: sql`excluded.liquidation_code`, managedIssueCode: sql`excluded.managed_issue_code`, isEtp: sql`excluded.is_etp`, isWarrant: sql`excluded.is_warrant`, isPreferred: sql`excluded.is_preferred`, isSuspended: sql`excluded.is_suspended`, enabled: true, rawPayload: sql`excluded.raw_payload`, missingRuns: 0, lastSeenAt: now, updatedAt: now } }); }; await upsert(krCommonStockUniverse, common); await upsert(krSpecialInstrumentUniverse, special); saved += rows.length; }
    for (let offset = 0; offset < uniqueUsRows.length; offset += 250) { const rows = uniqueUsRows.slice(offset, offset + 250).map((row) => { const acquisition = /acquisition|blank\s*check|spac/i.test(`${row.name} ${row.englishName}`); return { ...row, instrumentType: acquisition ? "SPAC" : row.isWarrant || row.isDerivative ? "DERIVATIVE" : row.isEtf ? "ETF" : row.isDr ? "DR" : "COMMON_STOCK", enabled: true, lastSeenAt: now, updatedAt: now }; }); const common = rows.filter((row) => row.instrumentType === "COMMON_STOCK" && !row.isEtf && !row.isWarrant && !row.isDerivative && !row.isDr && !row.isLeveraged && !row.isInverse); const special = rows.filter((row) => !common.includes(row)); const upsert = async (table: typeof usCommonStockUniverse, batch: typeof common) => { if (!batch.length) return; await db.insert(table).values(batch).onConflictDoUpdate({ target: [table.market, table.code], set: { realtimeSymbol: sql`excluded.realtime_symbol`, name: sql`excluded.name`, englishName: sql`excluded.english_name`, instrumentType: sql`excluded.instrument_type`, etpType: sql`excluded.etp_type`, securityType: sql`excluded.security_type`, currency: sql`excluded.currency`, isEtf: sql`excluded.is_etf`, isWarrant: sql`excluded.is_warrant`, isDerivative: sql`excluded.is_derivative`, isDr: sql`excluded.is_dr`, enabled: true, rawPayload: sql`excluded.raw_payload`, missingRuns: 0, lastSeenAt: now, updatedAt: now } }); }; await upsert(usCommonStockUniverse, common); await upsert(usSpecialInstrumentUniverse, special); saved += rows.length; }
