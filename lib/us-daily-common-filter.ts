...
export async function filterUsDailyCandidates<T extends Record<string, any>>(items: T[]) {
  ...
  if (pool && items.length) {
    ...
    for (const row of result.rows) metrics.set(`${row.market}:${row.code}`, { marketCap: row.market_cap == null ? null : Number(row.market_cap), turnoverRatio: row.turnover_ratio == null ? null : Number(row.turnover_ratio), tradingValue: row.trading_value == null ? null : Number(row.trading_value) });
    void keys;
  }
