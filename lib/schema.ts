...
export const marketRssArticles = pgTable(
  "market_rss_articles",
  {
    ...
    translationSourceLanguage: text("translation_source_language").notNull().default("en"),
    translationTargetLanguage: text("translation_target_language").notNull().default("ko"),
    translationCharCount: integer("translation_char_count").notNull().default(0),
    translationSkippedReason: text("translation_skipped_reason"),
    translationTranslatedAt: timestamp("translation_translated_at", { withTimezone: true }),
...
export const usInstrumentUniverseCandles = pgTable("us_instrument_universe_candles", {
  id: bigserial("id", { mode: "number" }).primaryKey(), market: text("market").notNull(), code: text("code").notNull(), timeframe: text("timeframe").notNull().default("D"), candleDate: text("candle_date").notNull(), periodEndDate: text("period_end_date"), candleTime: timestamp("candle_time", { withTimezone: true }), open: doublePrecision("open"), high: doublePrecision("high"), low: doublePrecision("low"), close: doublePrecision("close"), volume: doublePrecision("volume"), priceSign: text("price_sign"), priceDiff: doublePrecision("price_diff"), changeRate: doublePrecision("change_rate"), tradingValue: doublePrecision("trading_value"), rawPayload: text("raw_payload").notNull().default(""), source: text("source").notNull().default("KIS"), fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("us_instrument_universe_candles_unique").on(table.market, table.code, table.timeframe, table.candleDate), index("us_instrument_universe_candles_lookup_idx").on(table.market, table.code, table.timeframe, table.candleDate), index("us_instrument_universe_candles_refresh_idx").on(table.timeframe, table.fetchedAt, table.market, table.code), index("us_instrument_universe_candles_latest_date_idx").on(table.timeframe, table.candleDate)]);
...
export const krLatestDailyCandles = pgTable("kr_latest_daily_candles", {
  ...
  open: doublePrecision("open"), high: doublePrecision("high"), low: doublePrecision("low"), close: doublePrecision("close"), volume: doublePrecision("volume"),
  fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("kr_latest_daily_candles_market_code_unique").on(table.market, table.code), index("kr_latest_daily_candles_date_idx").on(table.candleDate)]);

export const usLatestDailyCandles = pgTable("us_latest_daily_candles", {
  market: text("market").notNull(), code: text("code").notNull(), candleDate: text("candle_date").notNull(),
  open: doublePrecision("open"), high: doublePrecision("high"), low: doublePrecision("low"), close: doublePrecision("close"), volume: doublePrecision("volume"),
  fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("us_latest_daily_candles_market_code_unique").on(table.market, table.code), index("us_latest_daily_candles_date_idx").on(table.candleDate)]);

const makeKrUniverseTable = (tableName: string, uniqueName: string, enabledIndexName: string) => pgTable(tableName, {
  id: bigserial("id", { mode: "number" }).primaryKey(),
...
}, (table) => [uniqueIndex(uniqueName).on(table.market, table.code), index(enabledIndexName).on(table.enabled, table.market, table.code)]);

export const krCommonStockUniverse = makeKrUniverseTable("kr_common_stock_universe", "kr_common_stock_universe_market_code_unique", "kr_common_stock_universe_enabled_idx");
export const krSpecialInstrumentUniverse = makeKrUniverseTable("kr_special_instrument_universe", "kr_special_instrument_universe_market_code_unique", "kr_special_instrument_universe_enabled_idx");
...
const makeUsUniverseTable = (tableName: string, uniqueName: string, enabledIndexName: string) => pgTable(tableName, {
...
export const usCommonStockUniverse = makeUsUniverseTable("us_common_stock_universe", "us_common_stock_universe_market_code_unique", "us_common_stock_universe_enabled_idx");
export const usSpecialInstrumentUniverse = makeUsUniverseTable("us_special_instrument_universe", "us_special_instrument_universe_market_code_unique", "us_special_instrument_universe_enabled_idx");
// Compatibility view retained under the historical name; reads are valid, writes must target a split table.
export const usInstrumentUniverse = makeUsUniverseTable("us_instrument_universe", "us_instrument_universe_market_code_unique", "us_instrument_universe_enabled_idx");
...
export const instrumentFundamentalHistory = pgTable("instrument_fundamental_history", { id: bigserial("id", { mode: "number" }).primaryKey(), market: text("market").notNull(), code: text("code").notNull(), name: text("name").notNull().default(""), price: doublePrecision("price"), changeRate: doublePrecision("change_rate"), open: doublePrecision("open"), high: doublePrecision("high"), low: doublePrecision("low"), volume: doublePrecision("volume"), tradingValue: doublePrecision("trading_value"), marketCap: doublePrecision("market_cap"), sharesOutstanding: doublePrecision("shares_outstanding"), currency: text("currency"), source: text("source").notNull(), rawPayload: text("raw_payload").notNull().default(""), observedAt: timestamp("observed_at", { withTimezone: true }).notNull(), fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull() }, (table) => [uniqueIndex("instrument_fundamental_history_market_code_observed_unique").on(table.market, table.code, table.observedAt), index("instrument_fundamental_history_lookup_idx").on(table.market, table.code, table.observedAt)]);
...
export const krMinuteCandles = pgTable("kr_minute_candles", {
  market: text("market").notNull(), code: text("code").notNull(), candleDate: text("candle_date").notNull(), candleTime: text("candle_time").notNull(),
  open: doublePrecision("open"), high: doublePrecision("high"), low: doublePrecision("low"), close: doublePrecision("close"), volume: doublePrecision("volume"), source: text("source").notNull().default("KIS"), fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("kr_minute_candles_pk").on(table.market, table.code, table.candleDate, table.candleTime), index("kr_minute_candles_lookup_idx").on(table.market, table.code, table.candleDate, table.candleTime)]);

export const krMinuteFetchLogs = pgTable("kr_minute_fetch_logs", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  market: text("market").notNull(), code: text("code").notNull(), requestDate: text("request_date").notNull(), requestTime: text("request_time").notNull(),
  httpStatus: integer("http_status"), responseCode: text("response_code"), responseMessage: text("response_message"), responseCount: integer("response_count"),
  requestUrl: text("request_url").notNull(), rawPayload: text("raw_payload").notNull().default(""), parsedPayload: text("parsed_payload").notNull().default(""),
  ok: boolean("ok").notNull().default(false), error: text("error"), fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("kr_minute_fetch_logs_lookup_idx").on(table.market, table.code, table.fetchedAt)]);

export const usMinuteCandles = pgTable("us_minute_candles", {
  market: text("market").notNull(), code: text("code").notNull(), candleDate: text("candle_date").notNull(), candleTime: text("candle_time").notNull(),
  open: doublePrecision("open"), high: doublePrecision("high"), low: doublePrecision("low"), close: doublePrecision("close"), volume: doublePrecision("volume"), source: text("source").notNull().default("KIS"), fetchedAt: timestamp("fetched_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("us_minute_candles_pk").on(table.market, table.code, table.candleDate, table.candleTime), index("us_minute_candles_lookup_idx").on(table.market, table.code, table.candleDate, table.candleTime)]);
