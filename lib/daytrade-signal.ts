...
export function evaluateDayTradeSignal(candles: DayTradeCandle[], options: { minCandles?: number; minRvol?: number; minTurnoverRatio?: number; marketCap?: number; tradingValue?: number } = {}): DayTradeSignal {
  ...
  if (rows.length < (options.minCandles ?? 35)) return { qualifies: false, state: "INSUFFICIENT_HISTORY", score: 0, signalDate: rows.at(-1)?.date ?? null, entryReference: null, indicators: empty, reasons: [], warnings: ["INSUFFICIENT_HISTORY", "NEXT_SESSION_OPEN_REQUIRED"] };
