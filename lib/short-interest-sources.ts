...
export function normalizeFinraShortVolume(ticker: string, row: Record<string, unknown> | null): ShortInterestMetric {
  ...
  return { ticker: ticker.toUpperCase(), shortVolume, totalVolume, shortVolumeRatio: shortVolume != null && totalVolume ? shortVolume / totalVolume * 100 : null, shortInterest: null, daysToCover: null, asOf, source: "FINRA", status, reason: status === "NULL_FIELD" ? "FINRA 응답 필수 수량 필드 누락" : undefined };
}
