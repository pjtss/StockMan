...
export type UsBollingerResult = {
  ...
  filter: { minPrice: boolean; minVolume: boolean; minTurnoverRatio: boolean; commonMarketCap: boolean; commonTurnoverRatio: boolean };
  error?: string;
...
export async function loadUsBollingerPolicy(moduleKey: "us-bollinger-band" | "us-bollinger-middle-lower" = "us-bollinger-band"): Promise<UsBollingerPolicy> {
  ...
  return {
    ...
    reboundAfterBreakout: policy?.reboundAfterBreakout !== false,
    reboundLookback: Math.max(1, Math.floor(Number(policy?.reboundLookback ?? 3))),
    reboundTolerancePercent: Math.max(0, Number(policy?.reboundTolerancePercent ?? 0.5)),
  };
}
...
export async function scanStoredUsBollingerBands(options: { policy?: Partial<UsBollingerPolicy>; concurrency?: number; context?: UsDailyScanContext; moduleKey?: "us-bollinger-band" | "us-bollinger-middle-lower" } = {}) {
  ...
  const policy: UsBollingerPolicy = {
    ...
    reboundAfterBreakout: configured.reboundAfterBreakout !== false,
    reboundLookback: Math.max(1, Math.floor(Number(configured.reboundLookback ?? 3))),
    reboundTolerancePercent: Math.max(0, Number(configured.reboundTolerancePercent ?? 0.5)),
  };
