...
export async function runUsObvScan() {
  ...
  if (db) {
    ...
    const sessionRows = await db.select({ market: usTurnoverRatioSnapshots.market, code: usTurnoverRatioSnapshots.code, name: usTurnoverRatioSnapshots.name }).from(usTurnoverRatioSnapshots).where(and(gte(usTurnoverRatioSnapshots.observedAt, sessionStart), lte(usTurnoverRatioSnapshots.observedAt, now)));
