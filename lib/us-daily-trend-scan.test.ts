...
describe("us daily trend scan", () => {
  it("uses the shared DB context and returns score policy metadata", async () => {
    ...
    expect(result.policy.minScore).toBe(100);
    expect(result.results).toHaveLength(1);
    expect(result.results[0]).toHaveProperty("scoreParts");
  });

  it("does not qualify a symbol when the OBV/ADL confirmation is missing", async () => {
    ...
    expect(result.results[0].rejectionReasons).toContain("obv_signal_not_increasing_or_obv_below_signal");
  });
});
