...
const defaultsByModule: Record<FeatureModuleKey, CommonModuleSettings> = {
  ...
  "kr-bollinger-band": { enabled: true, startTime: "08:00", endTime: "15:30", cooldownSeconds: 60, intervalSeconds: 600, activeDays: [1, 2, 3, 4, 5], featureSettings: { krBollingerPolicy: { period: 20, stdDevMultiplier: 2, minPrice: 0, minVolume: 0, minTurnoverRatio: 0, zone: "LOWER_OR_BELOW", requireObvAdlSignal: false, obvSignalPeriod: 9, adlSignalPeriod: 9 } } },
  "kr-golden-cross": { enabled: true, startTime: "00:00", endTime: "23:59", cooldownSeconds: 60, intervalSeconds: 86400, activeDays: [1, 2, 3, 4, 5, 6], featureSettings: { goldenCrossPolicy: { shortPeriod: 9, longPeriod: 20, recentCrossLookback: 5, approachingProximityPercent: 0.5, requireObvAboveSignal: true, requireAdlAboveSignal: true, obvSignalPeriod: 9, adlSignalPeriod: 9 } } },
  "kr-bollinger-middle-lower": { enabled: true, startTime: "08:00", endTime: "15:30", cooldownSeconds: 60, intervalSeconds: 600, activeDays: [1, 2, 3, 4, 5], featureSettings: { krBollingerPolicy: { period: 20, stdDevMultiplier: 2, minPrice: 0, minVolume: 0, minTurnoverRatio: 0, zone: "MIDDLE_TO_LOWER", requireObvAdlSignal: false, obvSignalPeriod: 9, adlSignalPeriod: 9 } } },
  "kr-daily-cache": { enabled: true, startTime: "08:00", endTime: "15:30", cooldownSeconds: 60, intervalSeconds: 86_400, activeDays: [1, 2, 3, 4, 5] },
...
export async function saveFeatureModuleSettings(key: FeatureModuleKey, settings: CommonModuleSettings) {
  ...
  if (key === "us-bollinger-band" || key === "us-bollinger-middle-lower") {
    ...
    for (const name of ["obvSignalPeriod", "adlSignalPeriod"]) { if (policy?.[name] !== undefined && (!Number.isInteger(Number(policy[name])) || Number(policy[name]) < 2 || Number(policy[name]) > 100)) throw new Error(`INVALID_BOLLINGER_${name.toUpperCase()}`); }
    if (policy?.requireObvAdlSignal !== undefined && typeof policy.requireObvAdlSignal !== "boolean") throw new Error("INVALID_BOLLINGER_OBV_ADL_POLICY");
    if (policy?.reboundAfterBreakout !== undefined && typeof policy.reboundAfterBreakout !== "boolean") throw new Error("INVALID_BOLLINGER_REBOUND_POLICY");
    if (policy?.reboundLookback !== undefined && (!Number.isInteger(Number(policy.reboundLookback)) || Number(policy.reboundLookback) < 1 || Number(policy.reboundLookback) > 30)) throw new Error("INVALID_BOLLINGER_REBOUND_LOOKBACK");
    if (policy?.reboundTolerancePercent !== undefined && (!Number.isFinite(Number(policy.reboundTolerancePercent)) || Number(policy.reboundTolerancePercent) < 0 || Number(policy.reboundTolerancePercent) > 10)) throw new Error("INVALID_BOLLINGER_REBOUND_TOLERANCE");
  ...
  if (key === "kr-bollinger-band" || key === "kr-bollinger-middle-lower") {
    ...
    for (const name of ["obvSignalPeriod", "adlSignalPeriod"]) { if (policy?.[name] !== undefined && (!Number.isInteger(Number(policy[name])) || Number(policy[name]) < 2 || Number(policy[name]) > 100)) throw new Error(`INVALID_KR_BOLLINGER_${name.toUpperCase()}`); }
    if (policy?.requireObvAdlSignal !== undefined && typeof policy.requireObvAdlSignal !== "boolean") throw new Error("INVALID_KR_BOLLINGER_OBV_ADL_POLICY");
    if (policy?.reboundAfterBreakout !== undefined && typeof policy.reboundAfterBreakout !== "boolean") throw new Error("INVALID_KR_BOLLINGER_REBOUND_POLICY");
    if (policy?.reboundLookback !== undefined && (!Number.isInteger(Number(policy.reboundLookback)) || Number(policy.reboundLookback) < 1 || Number(policy.reboundLookback) > 30)) throw new Error("INVALID_KR_BOLLINGER_REBOUND_LOOKBACK");
    if (policy?.reboundTolerancePercent !== undefined && (!Number.isFinite(Number(policy.reboundTolerancePercent)) || Number(policy.reboundTolerancePercent) < 0 || Number(policy.reboundTolerancePercent) > 10)) throw new Error("INVALID_KR_BOLLINGER_REBOUND_TOLERANCE");
  }
