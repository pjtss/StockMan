...
export async function findUsFiveDayHighBreakout({ code: rawCode, market: rawMarket, asOfDate }: UsFiveDayHighBreakoutRequest): Promise<UsFiveDayHighBreakoutResult> {
  ...
  if (!daily) return { ok: false, code, market, currentPrice: null, previousFiveDayHigh: null, previousFiveTradingDays: [], qualifies: false, daily: { ok: false, status: 0, candleCount: 0 }, price: { ok: false, status: 0 }, error: "KIS access token unavailable" };
  const previous = selectPreviousFiveTradingDays(daily.candles, asOfDate);
  if (!daily.ok || previous.length < 5) return { ok: false, code, market, currentPrice: null, previousFiveDayHigh: previous.length ? Math.max(...previous.map((candle) => candle.high)) : null, previousFiveTradingDays: previous.map((candle) => candle.date), qualifies: false, daily: { ok: daily.ok, status: daily.status, candleCount: daily.candles.length, rawText: daily.response.rawText.slice(0, 1000) }, price: { ok: false, status: 0 }, error: previous.length < 5 ? "fewer than five prior trading days" : "daily price API failed" };
