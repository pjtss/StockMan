...
export async function ensureUsInstrument(input: { market: string; code: string; name?: string }) {
  ...
  const [row] = await db.insert(usInstruments).values({ market, code, name: input.name?.trim() ?? "" })
    .onConflictDoUpdate({ target: [usInstruments.market, usInstruments.code], set: { name: input.name?.trim() ?? "", updatedAt: new Date() } })
