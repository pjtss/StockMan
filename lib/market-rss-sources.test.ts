...
describe("market RSS sources", () => { it("isolates source failures", async () => { const result = await fetchAllMarketRss(); expect(result.results.find((item) => item.source === "GLOBENEWSWIRE")?.ok).toBe(false); expect(result.results.filter((item) => item.ok)).toHaveLength(3); }); });
