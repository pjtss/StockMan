...
export async function scanMultiTimeframeBbPullback(market: "KR" | "US", mode: MultiTimeframeBbMode = "pullback") {
  const u = market === "US" ? "us_common_stock_universe" : "kr_common_stock_universe"; const c = market === "US" ? "us_instrument_universe_candles" : "kr_instrument_universe_candles";
  const eligibility = await queryEligibleUniverse(getDb(), market);
  const scopes = { rows: eligibility.rows };
  const candles = await getDb().execute(sql.raw(`SELECT candles.market,candles.code,candles.timeframe,candles.candle_date AS date,candles.high,candles.low,candles.close FROM ${c} candles JOIN ${u} universe ON universe.market = candles.market AND universe.code = candles.code WHERE candles.timeframe IN ('D','W','M') AND candles.close IS NOT NULL ORDER BY candles.market,candles.code,candles.timeframe,candles.candle_date`));
