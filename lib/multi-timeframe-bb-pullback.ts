...
export async function scanMultiTimeframeBbPullback(market: "KR" | "US") {
  ...
  const qualified = (scopes.rows as Scope[]).map((s) => { const g = map.get(`${s.market}:${s.code}`); if (!g) return null; const d = completed(g.D, "D"), w = completed(g.W, "W"), m = completed(g.M, "M"); const db = bb(d), wb = bb(w), mb = bb(m), last = d.at(-1), wLast = w.at(-1), mLast = m.at(-1); if (!last || !wLast || !mLast || !db || !wb || !mb) return null; const weeklyMiddleToUpper = wLast.close >= wb.middle && wLast.close <= wb.upper; const monthlyMiddleToUpper = mLast.close >= mb.middle && mLast.close <= mb.upper; const dailyAtOrBelowMiddle = last.close <= db.middle; if (!weeklyMiddleToUpper || !monthlyMiddleToUpper || !dailyAtOrBelowMiddle) return null; return { market: s.market, code: s.code, name: s.name, daily: { date: last.date, close: last.close, bbMiddle: db.middle, bbLower: db.lower, atOrBelowMiddle: true }, weekly: { date: wLast.date, close: wLast.close, bbMiddle: wb.middle, bbUpper: wb.upper, middleToUpper: true }, monthly: { date: mLast.date, close: mLast.close, bbMiddle: mb.middle, bbUpper: mb.upper, middleToUpper: true } }; }).filter(Boolean);
  return { ok: true, market, instrumentCount: scopes.rows.length, qualifiedCount: qualified.length, qualified, policy: { completedCandlesOnly: false, weeklyMonthly: "latest close between BB middle and BB upper", daily: "latest close <= BB middle", indicators: "BB position only; latest cached close is used as current/last traded price" } };
}
