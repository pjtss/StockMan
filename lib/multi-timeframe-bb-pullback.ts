...
export async function scanMultiTimeframeBbPullback(market: "KR" | "US") {
  ...
  const qualified = (scopes.rows as Scope[]).map((s) => { const g = map.get(`${s.market}:${s.code}`); if (!g) return null; const d = completed(g.D, "D"), w = completed(g.W, "W"), m = completed(g.M, "M"); const db = bb(d), wb = bb(w), mb = bb(m), wp = previousBb(w), mp = previousBb(m), last = d.at(-1), wLast = w.at(-1), mLast = m.at(-1), wPrev = w.at(-2), mPrev = m.at(-2); if (!last || !wLast || !mLast || !wPrev || !mPrev || !db || !wb || !mb || !wp || !mp) return null; const weeklyMiddleGoldenCross = wPrev.close <= wp.middle && wLast.close > wb.middle; const monthlyMiddleGoldenCross = mPrev.close <= mp.middle && mLast.close > mb.middle; if (!weeklyMiddleGoldenCross || !monthlyMiddleGoldenCross) return null; if (last.low > db.lower * 1.01 || last.close < db.lower) return null; return { market: s.market, code: s.code, name: s.name, daily: { date: last.date, close: last.close, low: last.low, bbLower: db.lower, touched: true, touchDistancePercent: Number(((last.low / db.lower - 1) * 100).toFixed(4)) }, weekly: { date: wLast.date, close: wLast.close, bbMiddle: wb.middle, previousClose: wPrev.close, previousBbMiddle: wp.middle, middleGoldenCross: true }, monthly: { date: mLast.date, close: mLast.close, bbMiddle: mb.middle, previousClose: mPrev.close, previousBbMiddle: mp.middle, middleGoldenCross: true } }; }).filter(Boolean);
  return { ok: true, market, instrumentCount: scopes.rows.length, qualifiedCount: qualified.length, qualified, policy: { completedCandlesOnly: true, weeklyMonthly: "previous close <= previous BB middle AND latest close > latest BB middle", daily: "low <= BB lower * 1.01 AND close >= BB lower", indicators: "BB middle golden cross + daily lower touch" } };
}
