...
export async function loadUserActivityDashboard(hours = 24, limit = 100, userKey = "") {
  ...
  const recentQuery = userKey
    ? pool.query('SELECT id, request_id AS "requestId", method, path, status_code AS "statusCode", user_key AS "userKey", ip_address AS ip, user_agent AS "userAgent", created_at AS "createdAt" FROM request_logs WHERE user_key=$1 ORDER BY created_at DESC LIMIT $2', [userKey, Math.min(safeLimit, 100)])
    : pool.query('SELECT id, request_id AS "requestId", method, path, status_code AS "statusCode", user_key AS "userKey", ip_address AS ip, user_agent AS "userAgent", created_at AS "createdAt" FROM request_logs ORDER BY created_at DESC LIMIT $1', [Math.min(safeLimit, 100)]);
