...
export async function runDbScreener(
  ...
  const rows = (
    await pool.query(
      `WITH ${latestCtes} SELECT u.market,u.code,u.name,u.enabled,f.market_cap,f.shares_outstanding,f.currency,c.candle_date,c.fetched_at,c.close,c.high,c.low,c.volume,c.timeframe FROM ${universeTable} u LEFT JOIN LATERAL (SELECT market_cap,shares_outstanding,currency FROM instrument_fundamental_snapshots f WHERE f.market=u.market AND f.code=u.code AND ($3::date IS NULL OR f.observed_at < ($3::date + INTERVAL '1 day')) ORDER BY f.observed_at DESC NULLS LAST, f.fetched_at DESC NULLS LAST LIMIT 1) f ON true JOIN market_latest ml ON ml.market=u.market JOIN instrument_daily_latest dl ON dl.market=u.market AND dl.code=u.code AND dl.candle_date=ml.candle_date JOIN LATERAL (SELECT recent.* FROM (SELECT * FROM ${candleTable} c WHERE c.market=u.market AND c.code=u.code AND c.timeframe=ANY($2) AND c.volume > 0 AND ($3::date IS NULL OR c.candle_date::date <= $3::date) ORDER BY c.timeframe,c.candle_date DESC LIMIT 120) recent ORDER BY recent.timeframe,recent.candle_date ASC) c ON true WHERE u.enabled=true AND u.daily_active=true AND u.instrument_type='COMMON_STOCK' AND u.market=ANY($1) ORDER BY u.market,u.code,c.timeframe,c.candle_date ASC`,
