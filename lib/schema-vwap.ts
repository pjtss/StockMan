...
export const usIntradayVwapAlerts = pgTable("us_intraday_vwap_alerts", {
  id: bigserial("id", { mode: "number" }).primaryKey(), market: text("market").notNull(), code: text("code").notNull(), sessionDate: text("session_date").notNull(), sentAt: timestamp("sent_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex("us_intraday_vwap_alert_market_code_date_unique").on(table.market, table.code, table.sessionDate)]);
