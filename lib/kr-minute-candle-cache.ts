...
export async function fetchKrMinuteCandles(code: string, minutes = 30, market = "KOSPI") {
  ...
  for (let page = 0; rows.length < limit && page < 40; page++) {
    ...
    const pageRows = output.map((r: any) => ({ date: String(r.stck_bsop_date ?? ""), time: String(r.stck_cntg_hour ?? ""), open: Number(r.stck_oprc ?? 0), high: Number(r.stck_hgpr ?? 0), low: Number(r.stck_lwpr ?? 0), close: Number(r.stck_prpr ?? 0), volume: Number(r.cntg_vol ?? 0), cumulativeTradingValue: Number(r.cntg_pbmn ?? r.acml_tr_pbmn ?? 0) || undefined, raw: r })).filter((r: any) => r.date && r.time && r.close > 0);
