...
export async function fetchKrDailyPrice(request: { code: string; startDate?: string; endDate?: string }): Promise<KrDailyPriceResponse | null> {
  ...
  const response = await fetch(url, { headers: { "content-type":"application/json", Authorization: buildKisAuthorization(token), appkey:KIS_APPKEY ?? "", appsecret:KIS_APPSECRET ?? "", tr_id:"FHKST03010100", custtype:"P" } });
  ...
  return { ok: response.ok && parsed?.rt_cd === "0", status: response.status, request: { url:url.toString(), headers:{ Authorization:"Bearer <masked>", appkey:"<masked>", appsecret:"<masked>", tr_id:"FHKST03010100" } }, response:{ rawText, parsed }, candles, diagnostics:{ httpStatus:response.status, kisOk:parsed?.rt_cd === "0", rtCd:parsed?.rt_cd ?? null, msgCd:parsed?.msg_cd ?? null, msg1:parsed?.msg1 ?? null, outputKey:"output2", rawOutputCount:rows.length, parsedCandleCount:candles.length, firstDate:candles[0]?.date ?? null, lastDate:candles.at(-1)?.date ?? null } };
}
