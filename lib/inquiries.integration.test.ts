...
const { addLike } = await import("./inquiries");
...
describe.skipIf(!envLine)("inquiry PostgreSQL integration", () => {
  it("serializes concurrent likes for the same user", async () => {
    ...
    try {
      const identity = { userKey: "concurrent-user", ip: "127.0.0.1", userAgent: "vitest" };
      const results = await Promise.all([addLike(id, identity), addLike(id, identity)]);
