...
export async function getAutomationIntervalSeconds() {
  ...
  } catch { return DEFAULT_AUTOMATION_INTERVAL_SECONDS; }
}
...
export async function saveMfiThreshold(threshold: number) {
  ...
  const settings = await loadFeatureModuleSettings("us-daily-indicators");
  await saveFeatureModuleSettings("us-daily-indicators", { ...settings, featureSettings: { ...settings.featureSettings, evaluation: { ...settings.featureSettings?.evaluation, mfiThreshold: value } } });
  return value;
}
