...
export async function toggleLike(id: number, userKey: string) { const pool=getPool(); const client=await pool.connect(); try { await client.query("BEGIN"); await client.query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))", [`inquiry-like:${id}:${userKey}`]); const existing=await client.query("SELECT id FROM inquiry_likes WHERE inquiry_id=$1 AND user_key=$2",[id,userKey]); if(existing.rowCount){await client.query("DELETE FROM inquiry_likes WHERE id=$1",[existing.rows[0].id]);} else {await client.query("INSERT INTO inquiry_likes(inquiry_id,user_key) VALUES($1,$2)",[id,userKey]);} await client.query("UPDATE inquiries SET like_count=(SELECT COUNT(*) FROM inquiry_likes WHERE inquiry_id=$1) WHERE id=$1",[id]); await client.query("COMMIT"); return !existing.rowCount; } catch(error){await client.query("ROLLBACK"); throw error;} finally{client.release();} }
