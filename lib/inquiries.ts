...
export async function listInquiries(limit = 30, offset = 0) {
  const result = await getPool().query(`SELECT i.id, i.title, i.author_key, i.ip_address, i.user_agent, i.view_count, i.like_count, i.created_at, i.is_notice, COUNT(c.id)::int AS comment_count FROM inquiries i LEFT JOIN inquiry_comments c ON c.inquiry_id=i.id WHERE i.is_notice = false AND i.deleted_at IS NULL GROUP BY i.id ORDER BY i.created_at DESC LIMIT $1 OFFSET $2`, [Math.min(limit, 100), Math.max(offset, 0)]);
  return result.rows;
}

export async function getInquiry(id: number) {
  const [inquiry, comments] = await Promise.all([
    getPool().query(`SELECT id,title,content,author_key,ip_address,user_agent,view_count,like_count,created_at,is_notice FROM inquiries WHERE id=$1 AND deleted_at IS NULL`, [id]),
...
export async function deleteInquiry(id: number) { await getPool().query("UPDATE inquiries SET deleted_at=NOW(), updated_at=NOW() WHERE id=$1 AND deleted_at IS NULL", [id]); }
export async function deleteComment(id: number) { await getPool().query("DELETE FROM inquiry_comments WHERE id=$1", [id]); }
export async function setInquiryNotice(id: number, isNotice: boolean) { await getPool().query("UPDATE inquiries SET is_notice=$2 WHERE id=$1", [id, isNotice]); }
