...
export async function sendUsMinuteBollingerBandSignals(results: UsMinuteBollingerResult[]) {
  ...
  const description = qualified.map((item) => `**${item.market} ${item.code}** ${item.name} · TOP ${item.rank} · 등락률 ${formatDisplayPercent(item.changeRate)}\n현재 ${formatDisplayNumber(item.currentPrice)} · 하단 ${formatDisplayNumber(item.lower)} · 중심 ${formatDisplayNumber(item.middle)} · 1분봉 ${item.pointCount}개`).join("\n\n").slice(0, 4050);
