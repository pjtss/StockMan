...
export async function sendUsBollingerBandSignals(results: UsBollingerResult[]) {
  ...
  const response = await fetch(`${webhook}${webhook.includes("?") ? "&" : "?"}wait=true`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(toTextWebhookPayload({ username: "STOCKMAN BOLLINGER", allowed_mentions: { parse: [] }, embeds: [{ title: "🚨 해외주식 일봉 볼린저밴드 하단 이탈 알림", description, color: 0x7c3aed, footer: { text: "STOCKMAN · 종가와 하단선 비교 · 카드 1개 통합" }, timestamp: new Date().toISOString() }] })) });
