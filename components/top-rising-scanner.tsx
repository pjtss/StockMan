...
export function TopRisingScanner({ current = "top-rising" }: { current?: "top-rising" | "us-top-rising" } = {}) {
  const isUs = current === "us-top-rising";
  ...
  const fetchStocks = async (isAuto = false) => {
    try {
      ...
      setError(null);
      const endpoint = isUs ? "/api/stock/us/top-rising" : "/api/stock/top-rising";
      ...
      <div className={styles.container}>
        ...
        <div className={styles.titleGroup}>
          <p className={styles.kicker}>LIVE QUANT TERMINAL</p>
            <h1>{isUs ? "미국 상승률 TOP N" : "한국 상승률 상위 N개 스캐너"}</h1>
            <p className={styles.subtitle}>
              {isUs
                ? "미국 주식 상승률 상위 종목을 60초마다 모니터링합니다."
                : "국내 주식 상승률 상위 종목을 60초마다 모니터링하고 DB에 저장/갱신합니다."}
            </p>
          </div>
          <div className={styles.actions}>
            ...
            <button className={styles.syncBtn} onClick={handleManualSync} disabled={loading || syncing}>
              {isUs ? "🔄 실시간 새로고침" : syncing ? "⚡ 동기화 중..." : "🔄 실시간 수동 갱신"}
            </button>
          </div>
        </div>

        {!isUs && isFallback && <div className={styles.warningAlert} style={{ background: "rgba(234, 179, 8, 0.1)", border: "1px solid rgba(234, 179, 8, 0.2)", color: "#eab308", padding: "12px 16px", borderRadius: "12px", fontWeight: "bold" }}>실시간 KIS API 장애 상태입니다. 현재 표시되는 데이터는 최종 캐시 데이터입니다.</div>}
