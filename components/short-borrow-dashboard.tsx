...
export function ShortBorrowDashboard() {
  ...
  return <main className="shortBorrowPage">
    ...
    {result && <section className="shortBorrowCard"><div className="shortBorrowCardHeader"><div><strong>{result.symbol}</strong><span>{result.borrowStatus} · {result.quoteStatus}</span></div><b className={`shortBorrowLevel ${levelClass[result.pressureLevel] || ""}`}>{result.pressureLevel} {result.pressureScore}</b></div><div className="shortBorrowGrid"><div><small>대차 가능 수량</small><strong>{formatDisplayNumber(result.availableQty)}</strong></div><div><small>수량 변화</small><strong>{formatDisplayPercent(result.availableQtyChangePercent)}</strong></div><div><small>주당 Locate</small><strong>{result.locatePricePerShare === null ? "-" : `$${formatDisplayNumber(result.locatePricePerShare)}`}</strong></div><div><small>Locate 비용률</small><strong>{formatDisplayPercent(result.locateFeeRatePercent)}</strong></div><div><small>Locate 가격 변화</small><strong>{formatDisplayPercent(result.locatePriceChangePercent)}</strong></div><div><small>견적 시각</small><strong>{result.quotedAt ? new Date(result.quotedAt).toLocaleString("ko-KR") : "-"}</strong></div></div><ul>{result.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul><p className="shortBorrowNotice">이 값은 미국 시장 전체가 아닌 현재 Alpaca 계정에 제공되는 대차 견적입니다. 실제 Locate 신청이나 공매도 주문은 수행하지 않습니다.</p></section>}
  </main>;
}
