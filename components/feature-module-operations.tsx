...
type ModuleSettings = {
  ...
  activeDays: number[];
  featureSettings?: {
    discordFormat?: { webhookUrl?: string };
...
export function FeatureModuleOperations({ moduleKey }: { moduleKey: FeatureModuleKey }) {
  ...
  return <section className={styles.panel}>
    ...
    <div className={styles.fields}><label>시작 시각<input type="time" value={settings.startTime} onChange={(event) => setSettings({ ...settings, startTime: event.target.value })} /></label><label>종료 시각<input type="time" value={settings.endTime} onChange={(event) => setSettings({ ...settings, endTime: event.target.value })} /></label><label>알림 쿨다운(초)<input type="number" min="0" value={settings.cooldownSeconds} onChange={(event) => setSettings({ ...settings, cooldownSeconds: Number(event.target.value) })} /></label>{moduleKey === "us-daily-indicators" && <label>실행 간격(초)<input type="number" min="60" value={settings.intervalSeconds ?? 600} onChange={(event) => setSettings({ ...settings, intervalSeconds: Number(event.target.value) })} /></label>}</div>
