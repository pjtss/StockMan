...
export function FeatureModuleOperations({ moduleKey }: { moduleKey: FeatureModuleKey }) {
  ...
  return <section className={styles.panel}>
    ...
    {(moduleKey === "kr-daily-cache" || moduleKey === "us-daily-cache") && <div className={styles.manualCache}><h3>필요할 때 수동 갱신</h3><p>관리자 요청으로 즉시 백그라운드 갱신합니다. 화면을 닫아도 서버 작업은 계속됩니다.</p><button type="button" onClick={() => void refreshDailyCache()} disabled={manualJob?.status === "PROCESSING" || manualJob?.status === "PENDING"}>{manualJob?.status === "PROCESSING" || manualJob?.status === "PENDING" ? "갱신 진행 중…" : "지금 일봉 캐시 갱신"}</button>{manualJob && <p className={styles.scheduleHint}>상태: {manualJob.status} · 처리: {manualJob.processedCount ?? 0}/{manualJob.instrumentCount ?? "-"} · 성공: {manualJob.successCount ?? 0} · 실패: {manualJob.failureCount ?? 0}</p>}</div>}
    ...
    {(moduleKey === "us-bollinger-band" || moduleKey === "us-bollinger-middle-lower") && <><h3>일봉 볼린저밴드 {moduleKey === "us-bollinger-middle-lower" ? "중단선~하단선" : "하단 이탈"} 필터</h3><div className={styles.fields}><label>기간(거래일)<input type="number" min="2" max="200" value={Number(bollingerPolicy.period ?? 20)} onChange={(event) => updateBollingerPolicy("period", Number(event.target.value))} /></label><label>표준편차 배수<input type="number" min="0.1" max="10" step="0.1" value={Number(bollingerPolicy.stdDevMultiplier ?? 2)} onChange={(event) => updateBollingerPolicy("stdDevMultiplier", Number(event.target.value))} /></label><label>최소 주가(USD)<input type="number" min="0" step="0.01" value={Number(bollingerPolicy.minPrice ?? 0)} onChange={(event) => updateBollingerPolicy("minPrice", Number(event.target.value))} /></label><label>최소 거래량<input type="number" min="0" value={Number(bollingerPolicy.minVolume ?? 0)} onChange={(event) => updateBollingerPolicy("minVolume", Number(event.target.value))} /></label><label>시총 대비 거래대금 최소(%)<input type="number" min="0" step="0.1" value={Number(bollingerPolicy.minTurnoverRatio ?? 0)} onChange={(event) => updateBollingerPolicy("minTurnoverRatio", Number(event.target.value))} /></label></div><p className={styles.scheduleHint}>{moduleKey === "us-bollinger-middle-lower" ? "최근 저장 일봉의 종가가 중단선과 하단선 사이인 종목만 통과합니다." : "최근 저장 일봉의 종가가 하단선 이하인 종목만 통과합니다."}</p></>}
    ...
    {moduleKey === "kr-bollinger-middle-lower" && <><h3>국내 일봉 볼린저밴드 중단선~하단선 필터</h3><div className={styles.fields}><label>기간(거래일)<input type="number" min="2" max="200" value={Number(krBollingerPolicy.period ?? 20)} onChange={(event) => updateKrBollingerPolicy("period", Number(event.target.value))} /></label><label>표준편차 배수<input type="number" min="0.1" max="10" step="0.1" value={Number(krBollingerPolicy.stdDevMultiplier ?? 2)} onChange={(event) => updateKrBollingerPolicy("stdDevMultiplier", Number(event.target.value))} /></label><label>최소 주가(원)<input type="number" min="0" value={Number(krBollingerPolicy.minPrice ?? 0)} onChange={(event) => updateKrBollingerPolicy("minPrice", Number(event.target.value))} /></label><label>최소 거래량<input type="number" min="0" value={Number(krBollingerPolicy.minVolume ?? 0)} onChange={(event) => updateKrBollingerPolicy("minVolume", Number(event.target.value))} /></label><label>시총 대비 거래대금 최소(%)<input type="number" min="0" step="0.1" value={Number(krBollingerPolicy.minTurnoverRatio ?? 0)} onChange={(event) => updateKrBollingerPolicy("minTurnoverRatio", Number(event.target.value))} /></label></div><div className={styles.fields}><label>조회 봉 주기<select value={krBollingerPolicy.timeframe ?? "D"} onChange={(event) => updateKrBollingerPolicy("timeframe", event.target.value as "D" | "W" | "M")}><option value="D">일봉</option><option value="W">주봉</option><option value="M">월봉</option></select></label></div><p className={styles.scheduleHint}>DB에 저장된 국내 일·주·월봉의 종가가 하단선 이상, 중단선 이하인 종목을 탐지합니다.</p></>}
