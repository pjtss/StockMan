...
## 2026-09-06
- **[차트 종목 탐색]** `/charts`의 차트 모달에 현재 결과 목록 기준 이전·다음 종목 이동과 위치 표시를 추가했다. 추출 결과가 있으면 현재 정렬된 결과 목록을, 없으면 입력 티커 목록을 탐색 대상으로 사용한다.
- **[차트 인접 종목 선조회]** 차트 모달은 현재 종목 데이터만 즉시 표시하고 앞·뒤 최대 2개 종목의 일봉 차트만 백그라운드 선조회·메모이즈한다. 전체 목록 일괄 다운로드보다 다음 종목 전환 지연과 호출량·메모리 사용량을 균형화한다.
- **[Next workspace root 고정]** 상위 디렉터리의 다른 `package-lock.json`을 Next.js가 workspace root로 오인하지 않도록 `outputFileTracingRoot`를 프로젝트 실행 경로로 고정했다. 로컬·프로덕션 빌드와 API 성능 측정 환경의 경로 재현성을 높인다.
- **[종목 추출 API 반복 성능 측정]** 로컬 API를 읽기 전용으로 반복 호출해 평균·p50·p95의 DB 로드·지표 계산·전체·실제 벽시계 시간을 출력하는 `scripts/measure-accumulation-api.mjs`와 `measure:accumulation` 명령을 추가했다. 단일 요청 변동과 서버 처리시간을 구분할 수 있다.
- **[종목 추출 중복 실행 방지]** 같은 시장·필터·분 단위 기준시각으로 동시에 들어온 매집 스캔 요청은 진행 중인 Promise를 공유해 페이지 재요청이나 중복 호출이 동일한 120봉 DB 조회와 계산을 반복하지 않게 했다.
- **[종목 추출 응답 속도 우선 캐시]** 동일 조건의 완료된 매집 스캔 결과를 프로세스 메모리에 15초간 재사용해 연속 새로고침·화면 재요청에서 DB 조회와 지표 계산을 생략한다. 테스트 환경에서는 결과 캐시를 비활성화하고, 기준시각·시장·필터가 다른 요청은 공유하지 않는다.
- **[종목 추출 결과 캐시 메모리 제한]** 응답 속도용 결과 캐시를 최대 32개 엔트리로 제한하고 오래된 키부터 제거해 필터 조합 증가에 따른 메모리 무한 증가를 방지했다.
- **[종목 추출 prepared statement]** 국내·해외 일봉 스캔 조회에 시장별 PostgreSQL prepared statement 이름을 부여해 반복 DB 요청에서 SQL 파싱·계획 재사용을 유도한다. 동적 테이블명은 시장별 고정 허용 목록에서만 선택한다.
- **[종목 추출 후보 인덱스 최적화]** 활성·일봉 대상·보통주 조건을 PostgreSQL 부분 인덱스에 반영해 국내·해외 일봉 스크리너가 비활성·특수종목을 후보 단계에서 함께 훑는 비용을 줄였다. 기존 인덱스와 충돌하지 않도록 `IF NOT EXISTS`로 추가했다.
- **[종목 추출 API 성능 관측성]** 국내·해외 매집 추출 API에 `Server-Timing`과 단계별 `X-Scan-*` 응답 헤더를 추가해 DB 로드·지표 계산·전체 소요시간을 브라우저와 CLI에서 동일하게 측정할 수 있게 했다. 결과 JSON의 기존 `timings` 계약은 유지한다.
- **[요청 로그 DB 오류 제거]** Geo ASN이 비어 있을 때 정수형 `request_logs.geo_asn`에 빈 문자열을 저장해 모든 요청 로그가 503이 되던 문제를 `null` 정규화로 수정하고 회귀 테스트를 추가했다. 스캔 API의 성능 측정 결과와 부가 로그 오류가 섞이지 않도록 요청 로그 저장을 안정화했다.
- **[KIS 매집 탐지 설계안]** KIS 국내·해외 기간별 시세를 원천 데이터로 수집하고, 인증·캐시·최신성 검증과 국내·해외 공통 v2 판정 엔진을 분리하는 설계안을 `docs/KIS_ACCUMULATION_DESIGN.md`에 제출했다. KIS에 없는 매집 판정을 임의로 API 기능처럼 표현하지 않고 OHLCV 휴리스틱으로 명시했다.

## 2026-09-06
- **[매집 탐지 v2]** 국내·해외의 계산·검증·점수·응답을 공통 파이프라인으로 통합했다. 직전 20봉 거래량 SMA20 기반 RVOL, 실제 최근 교차, 거래량 지속성, 하락일 종가 회복, EMA ATR 수축과 단발 급증 감점을 추가했다.
- **[캐시·설명 가능성]** 시장별 유효 캐시 거래일 일치, 장마감 이후 갱신, OHLCV 유효성 및 국내 시총 300억 초과를 검증하고 전체/제외/반환 건수와 점수 근거·모든 사용 봉 메타데이터를 반환한다.
- **[화면 연결]** 현재 국내/해외 선택을 존중하고 점수순 정렬, 계산 기준·제외 요약·봉별 갱신시각을 표시한다. 정책·공식 근거·한계는 docs/ACCUMULATION_V2.md에 기록했다.
...
- **[완료 알림 재시도]** 자동화 완료 Discord 알림이 503/429 또는 네트워크 오류를 만나면 최대 3회 재시도하고, 성공 결과에 실제 시도 횟수를 기록한다. 영구적인 4xx 오류는 불필요하게 반복하지 않는다.
