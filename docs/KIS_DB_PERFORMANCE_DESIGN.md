...
기준일 최신봉 CTE는 종목·날짜 인덱스를 활용하는 `DISTINCT ON (market, code) ... ORDER BY market, code, candle_date DESC` 방식으로 개선했다. 동일한 CTE에서 시장별 최신일을 도출해 시장·종목별 최신일 집계의 중복도 제거했다. 변경 후 별도 계획 측정값은 KR 종목별 `69.019ms`, US 종목별 `170.331ms`였다. 시장별 측정은 CTE 재사용 여부에 따라 달라질 수 있으므로, 추후에는 전체 스크리너 요청 단위의 p50/p95로 검증한다.

누적 매집의 첫 로컬 API 요청은 종목별 캔들 `LATERAL` 반복 조회를 전체 대상 종목의 단일 `ROW_NUMBER()` 집계로 변경한 뒤, 동일 조건 측정에서 로드 시간이 `13,475ms → 5,739ms`, 전체 시간이 `16,819ms → 9,106ms`로 감소했다. 이후 요청은 기존 15초 결과 캐시로 약 80~105ms를 유지했다. 측정은 로컬 DB와 동일 API를 사용한 단일 콜드 요청 포함 5회 표본이며, 운영 성능은 별도 p50/p95 관측으로 확인한다.
