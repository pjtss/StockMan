...
모든 수집·탐지 기능은 아래 신규 유니버스 테이블만 사용한다. 레거시 통합 테이블과 소비자는 V70에서 제거했다.
...
국내 레거시 테이블과 분리된 독립 테이블이며, 외래키는 신규 캐시 테이블에만 연결한다.
...
해외 레거시 테이블과 분리된 독립 테이블이며, 외래키는 신규 캐시 테이블에만 연결한다.
...
V70 마이그레이션에서 레거시 테이블을 삭제했다. 이후에는 신규 테이블의 시장별 건수, 캐시 성공률, 탐지 결과를 진단 API로 검증한다.
