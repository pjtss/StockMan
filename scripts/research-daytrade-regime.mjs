...
try {
  ...
  for (const rows of groups.values()) for (let i = 35; i < rows.length - 1; i += 1) { const date = rows[i].date; const closes = rows.slice(0, i + 1).map((x) => x.close); const current = ema(closes, 20); const prior = ema(closes.slice(0, -5), 20); const state = breadth.get(date) ?? { total: 0, rising: 0 }; state.total += 1; if (current > prior && rows[i].close > current) state.rising += 1; breadth.set(date, state); }
  const stats = new Map();
