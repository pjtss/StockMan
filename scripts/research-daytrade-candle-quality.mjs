...
const outcome = (rows, index) => { const entryRow = rows[index + 1]; if (!entryRow || entryRow.open <= 0) return null; const entry = entryRow.open * (1 + fee); const end = Math.min(rows.length - 1, index + 1); const up = rows[index + 1].high >= entry * 1.01, down = rows[index + 1].low <= entry * 0.995; if (up && down) return -0.005 - fee; if (up) return 0.01 - fee; if (down) return -0.005 - fee; return rows[end].close / entry - 1 - fee; };
...
try {
  ...
  const results = configs.map(({ closeLocation, emaDistance }) => { const returns = []; for (const rows of groups.values()) for (let i = 35; i < rows.length - 1; i += 1) if (signal(rows, i, closeLocation, emaDistance)) { const value = outcome(rows, i); if (value !== null) returns.push(value); } const wins = returns.filter((value) => value > 0).length; return { closeLocation, maxEmaDistance: emaDistance, signals: returns.length, winRate: returns.length ? wins / returns.length : 0, averageReturn: returns.length ? returns.reduce((sum, value) => sum + value, 0) / returns.length : 0, totalReturn: returns.reduce((sum, value) => sum + value, 0) }; }).filter((item) => item.signals >= 30).sort((a, b) => b.averageReturn - a.averageReturn);
  console.log(JSON.stringify({ market, days, feeBps: fee * 10000, sourceRows: result.rowCount, instruments: groups.size, entry: "next session open", target: "1%", stop: "0.5%", results }, null, 2));
